...
import {CustomSqlOptProps} from '../../types/Editor';
...
const CustomSqlOpt: FC<CustomSqlOptProps> = props => {
  ...
  return (
    <ListItem classes={{gutters: classes.customGutters}} button divider>
      <ListItemText
        ...
        primary={
          <span className={classes.option} onMouseDown={(e: any) => onClick()}>
            <span className={classes.optionLabel}>{placeholder}</span>
            <span className={classes.customIcon}>F(x)</span>
          </span>
        }
      />
    </ListItem>
