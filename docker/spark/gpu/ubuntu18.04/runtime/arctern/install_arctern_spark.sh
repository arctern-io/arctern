...
conda install -y -q -n arctern -c conda-forge -c file://${SCRIPTS_DIR}/conda-bld ${LIBARCTERN_FILE} ${PYARCTERN_FILE} ${ARCTERN_SPARK_FILE}
else
...
conda install -y -q -n arctern pyyaml shapely
conda clean --all -y
