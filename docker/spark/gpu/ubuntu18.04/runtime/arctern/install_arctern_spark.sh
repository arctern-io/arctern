...
conda install -y -q -n arctern -c conda-forge -c arctern -c file://${SCRIPTS_DIR}/conda-bld ${LIBARCTERN_FILE} ${PYARCTERN_FILE} ${ARCTERN_SPARK_FILE}
else
conda install -y -q -n arctern -c conda-forge -c arctern -c ${ARCTERN_CHANNEL}/label/cuda10.0 ${LIBARCTERN_FILE}
