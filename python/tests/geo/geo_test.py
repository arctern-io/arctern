...
def test_ST_Equals():
    ...
    array1 = pyarrow.array(data1)
    array2 = pyarrow.array(data2)
    rst = arctern.ST_Equals(array1, array2)
    assert rst[0] == 1
    assert rst[1] == 0
...
def test_ST_Touches():
    data1 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    data2 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((2 1,3 1,3 2,2 2,2 1))"])
    rst = arctern.ST_Touches(pyarrow.array(data1), pyarrow.array(data2))
    assert rst[0] == 0
    assert rst[1] == 1
...
def test_ST_Overlaps():
    data1 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    data2 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((2 1,3 1,3 2,2 2,2 1))"])
    rst = arctern.ST_Overlaps(pyarrow.array(data1), pyarrow.array(data2))
    assert rst[0] == 0
    assert rst[1] == 0
...
def test_ST_Crosses():
    data1 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    data2 = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((2 1,3 1,3 2,2 2,2 1))"])
    rst = arctern.ST_Crosses(pyarrow.array(data1), pyarrow.array(data2))
    assert rst[0] == 0
    assert rst[1] == 0

def test_ST_IsSimple():
    data = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    rst = arctern.ST_IsSimple(pyarrow.array(data))
    assert rst[0] == 1
    assert rst[1] == 1

def test_ST_GeometryType():
    data = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    rst = arctern.ST_GeometryType(pyarrow.array(data))
...
def test_ST_SimplifyPreserveTopology():
    data = pandas.Series(["POLYGON ((1 1,1 2,2 2,2 1,1 1))", "POLYGON ((1 1,1 2,2 2,2 1,1 1))"])
    rst = arctern.ST_SimplifyPreserveTopology(pyarrow.array(data), 10000)
    assert rst[0] == "POLYGON ((1 1,1 2,2 2,2 1,1 1))"

def test_ST_Point():
    data1 = pandas.Series([1.3, 2.5])
    data2 = pandas.Series([3.8, 4.9])
    string_ptr = arctern.ST_Point(pyarrow.array(data1), pyarrow.array(data2))
...
def test_ST_GeomFromGeoJSON():
    ...
    str_ptr = arctern.ST_GeomFromGeoJSON(pyarrow.array(data))
...
def test_ST_Contains():
    ...
    rst = arctern.ST_Contains(pyarrow.array(data2), pyarrow.array(data1))
    assert rst[0] == 1
    assert rst[1] == 0
    assert rst[2] == 1
    assert rst[3] == 0
...
def test_ST_Intersects():
    ...
    rst = arctern.ST_Intersects(pyarrow.array(data2), pyarrow.array(data1))
    assert rst[0] == 1
    assert rst[1] == 1
    assert rst[2] == 1
    assert rst[3] == 0
...
def test_ST_Within():
    ...
    rst = arctern.ST_Within(pyarrow.array(data2), pyarrow.array(data1))
    assert rst[0] == 0
    assert rst[1] == 0
    assert rst[2] == 0
    assert rst[3] == 0
...
def test_ST_Distance():
    ...
    rst = arctern.ST_Distance(pyarrow.array(data2), pyarrow.array(data1))
...
def test_ST_HausdorffDistance():
    ...
    rst = arctern.ST_HausdorffDistance(array1, array2)
...
def test_ST_ConvexHull():
    ...
    array = pyarrow.array(pandas.Series(data))
    rst = arctern.ST_ConvexHull(array)
...
def test_ST_Transform():
    data = ["POINT (10 10)"]
    array = pyarrow.array(pandas.Series(data))
    rst = arctern.ST_Transform(array, b"EPSG:4326", b"EPSG:3857")
...
def test_ST_Buffer():
    ...
    array = pyarrow.array(pandas.Series(data))
    rst = arctern.ST_Buffer(array, 1.2)
...
def test_ST_Union_Aggr():
    ...
    rst = arctern.ST_Union_Aggr(pyarrow.array(data))
...
def test_ST_Envelope_Aggr():
    ...
    p2 = "POLYGON ((5 1,7 1,7 2,5 2,5 1))"
    data = pandas.Series([p1, p2])
    rst = arctern.ST_Envelope_Aggr(pyarrow.array(data))
