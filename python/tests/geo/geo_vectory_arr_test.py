...
def test_suite():
    ...
    import time
    p1 = Process(target=wrapper_for_docker_test1)
    p2 = Process(target=wrapper_for_docker_test2)
...
# This function is only for ci test to verify whether error code 137 is caused by docker memory limit
def wrapper_for_docker_test1():
    ST_Intersection()
    ST_Equals()
    ST_Touches()
    ST_Overlaps()
