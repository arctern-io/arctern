...
df = pd.read_csv("/tmp/0_10_nyc_taxi_and_building.csv",
                 dtype=nyc_schema,
...
gpd_df['test_GeoSeries'] = gpd.GeoSeries(df['buildingtext_dropoff'].apply(shapely.wkt.loads)).apply(lambda x: x.to_wkb())
pd.testing.assert_frame_equal(arctern_df, gpd_df, check_dtype=False)

def trans2wkb(df, key,index=range(0,0)):
    if isinstance(index,range):
        index = range(0,df[key].count())
    import pygeos
    s_arr = []
    ...
    try:
        ...
        df[key] = pd.Series(s_arr,index=index)
    except:
        return None
    return df
...
# def test_lt():
#     try:
#         arctern_lt_df = arctern_df.lt(df)
#     except Exception as e1:
#         arctern_lt_exception = e1
#
#     try:
#         gpd_lt_df = gpd_df.lt(df)
#     except Exception as e2:
#         gpd_lt_exception = e2
#
#     assert type(gpd_lt_exception) == type(arctern_lt_exception)


# def test_gt():
#     try:
#         arctern_gt_df = arctern_df.gt(df)
#     except Exception as e1:
#         arctern_gt_exception = e1
#
#     try:
#         gpd_gt_df = gpd_df.gt(df)
#     except Exception as e2:
#         gpd_gt_exception = e2
#
#     assert type(gpd_gt_exception) == type(arctern_gt_exception)


# def test_le():
#     try:
#         arctern_le_df = arctern_df.le(df)
#     except Exception as e1:
#         arctern_le_exception = e1
#
#     try:
#         gpd_le_df = gpd_df.le(df)
#     except Exception as e2:
#         gpd_le_exception = e2
#
#     assert type(gpd_le_exception) == type(arctern_le_exception)


# def test_ge():
#     try:
#         arctern_ge_df = arctern_df.ge(df)
#     except Exception as e1:
#         arctern_ge_exception = e1
#
#     try:
#         gpd_ge_df = gpd_df.ge(df)
#     except Exception as e2:
#         gpd_ge_exception = e2
#
#     assert type(gpd_ge_exception) == type(arctern_ge_exception)
...
def test_abs():
    ...
    try:
        gpd_abs_df = (gpd.GeoSeries(df['buildingtext_dropoff'].apply(shapely.wkt.loads)).apply(lambda x: x.to_wkb())).abs()
    except Exception as e2:
...
def test_clip():
    ...
    try:
        gpd_clip_df = (gpd.GeoSeries(df['buildingtext_dropoff'].apply(shapely.wkt.loads)).apply(lambda x: x.to_wkb())).clip(-4, 6)
    except Exception as e2:
...
def test_pct_change():
    ...
    try:
        gpd_pct_change_df = (gpd.GeoSeries(df['buildingtext_dropoff'].apply(shapely.wkt.loads)).apply(lambda x: x.to_wkb())).pct_change()
    except Exception as e2:
...
def test_head():
    arctern_head_df = arctern_df.head(5)
    gpd_head_df = gpd_df.head(5)

    pd.testing.assert_frame_equal(arctern_head_df, gpd_head_df, check_dtype=False)
...
def test_tail():
    arctern_tail_df = arctern_df.tail(5)
    gpd_tail_df = gpd_df.tail(5)

    pd.testing.assert_frame_equal(arctern_tail_df, gpd_tail_df, check_dtype=False)
...
def test_pivot():
    arctern_pivot_df = arctern_df.pivot(index='tpep_pickup_datetime', columns='passenger_count', values='test_GeoSeries')
...
def test_pivot_table():
    arctern_pivot_table_df = pd.pivot_table(arctern_df, values= 'test_GeoSeries', index=['tpep_pickup_datetime'], columns=['passenger_count'], aggfunc=np.sum)
    gpd_pivot_table_df = pd.pivot_table(gpd_df, values= 'test_GeoSeries', index=['tpep_pickup_datetime'], columns=['passenger_count'], aggfunc=np.sum)
...
def test_dot():
    dot_series = pd.Series([1] * 17, index=[ "VendorID","tpep_pickup_datetime","tpep_dropoff_datetime","passenger_count","trip_distance","pickup_longitude","pickup_latitude","dropoff_longitude","dropoff_latitude","fare_amount","tip_amount","total_amount","buildingid_pickup","buildingid_dropoff","buildingtext_pickup","buildingtext_dropoff", "test_GeoSeries"])
    try:
...
def test_info():
    ...
    geo_res1 = geo_df.info()
    pd_res1 = pd_df.info()
    assert (geo_res1.__eq__(pd_res1))
