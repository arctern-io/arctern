...
class Marks(RootMarks):
    ...
    class Encode:
        ...
        def __init__(self, bounding_box: Value, color_gradient: Value,
                     square_size: Value, square_spacing: Value, opacity: Value, coordinate_system: Value, aggregation_type: Value):
            if not (isinstance(bounding_box.v, list)
                    and isinstance(color_gradient.v, list)
                    and isinstance(square_size.v, int)
                    and isinstance(square_spacing.v, int)
            ...
            self._square_spacing = square_spacing
        ...
        def to_dict(self):
            dic = {
                "enter": {
                    ...
                    "square_size": self._square_size.to_dict(),
                    "square_spacing": self._square_spacing.to_dict(),
...
class VegaSquareMap:
    def __init__(self, width: int, height: int, bounding_box: list, color_gradient: list,
                 square_size: int, square_spacing: int, opacity: float, coordinate_system: str, aggregation_type: str):
        self._width = width
        self._height = height
        self._bounding_box = bounding_box
        self._square_size = square_size
    ...
    def build(self):
        description = Description(desc="square_map_2d")
        ...
        encode = Marks.Encode(bounding_box=Marks.Encode.Value(self._bounding_box),
                              ...
                              square_size=Marks.Encode.Value(self._square_size),
                              square_spacing=Marks.Encode.Value(self._square_spacing),
    ...
    def square_spacing(self):
        return self._square_spacing
