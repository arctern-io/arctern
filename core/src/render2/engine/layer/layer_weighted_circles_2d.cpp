...
LayerWeightedCircles2D::LayerWeightedCircles2D()
...
void LayerWeightedCircles2D::Init() {
...
void LayerWeightedCircles2D::Shader() {
    ...
    const char *vertexShaderSource = \
        ...
        "layout (location = 2) in float pointSizeVec;\n"
        "layout (location = 3) in vec4 point_color;\n"
        "layout (location = 4) uniform vec2 screen_info;\n"
        "out vec4 color;\n"
        "void main()\n"
        "{\n"
        "   float tmp_x = posX;\n"
    ...
    glBufferData(GL_ARRAY_BUFFER, num_vertices_ * sizeof(float), pointsize_, GL_STATIC_DRAW);
    ...
    glUniform2f(4, window_params_.width(), window_params_.height());
    glBindVertexArray(VAO);

}

void LayerWeightedCircles2D::Render() {
...
void LayerWeightedCircles2D::set_vertices_colors() {
    ...
    pointsize_ = (float *)mem_pool.Malloc(num_vertices_ * 4);
