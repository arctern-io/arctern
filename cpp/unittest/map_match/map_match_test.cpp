...
std::vector<OGRGeometry*> construct_geometry(const std::vector<std::string>& wkt) {
    ...
    for (int32_t i = 0; i < wkt.size(); i++) {
        ...
        auto err_code = OGRGeometryFactory::createFromWkt(wkt[i].c_str(),
                                                          nullptr,
                                                          &gps_point);
...
TEST(MAP_MATCH_TEST, NEAREST_LOCATION_ON_ROAD) {
    ...
    for (int32_t i = 0; i < compare_result.size(); i++) {
        ...
        OGRGeometryFactory::createFromWkb(result_1->GetString(i).c_str(),
                                          nullptr,
                                          &projection_point);
...
TEST(MAP_MATCH_TEST, NEAREST_ROAD) {
    ...
    auto result = arctern::map_match::nearest_road(roads_binary_vec,
                                                   gps_points_binary_vec);
