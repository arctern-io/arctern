...
//    gps_points.push_back("POINT (1.0001 0)");
//
...
//
//    auto result = arctern::map_match::snap_to_road(roads_binary_vec, gps_points_binary_vec);
...
//        if (err_code != OGRERR_NONE) throw nullptr;
//        auto gps_point1 = dynamic_cast<OGRPoint *>(gps_point);
//        auto point1_x = gps_point1->getX();
//        auto point1_y = gps_point1->getY();
//
...
//        auto projection_point1_x = projection_point1->getX();
//        auto projection_point1_y = projection_point1->getY();
...
TEST(MAP_MATCH_TEST, SNAP_TO_ROAD2){
    std::ifstream in("/home/xge/nyc/nyc_road.csv");
    std::vector<std::string> roads;
    std::vector<std::string> gps_points;
    std::string line;
    ...
    std::ifstream in_1("/home/xge/nyc/nyc_point.csv");

    while (getline(in_1, line) ) {
        gps_points.push_back(line);
    }
    ...
    std::vector<std::shared_ptr<arrow::Array>> projections;
    ...
    std::shared_ptr<arrow::Array> road;
    builder.Finish(&road);
    std::vector<std::shared_ptr<arrow::Array>> roads_binary;
    roads_binary.push_back(road);

//    auto xxx_road = std::static_pointer_cast<arrow::BinaryArray>(roads_binary[0]);
//    std::cout << "The num of roads is " << xxx_road->length() <<  std::endl;
//
//    auto xxx_gps = std::static_pointer_cast<arrow::BinaryArray>(projections[0]);
//    std::cout << "The num of roads is " << xxx_gps->length() <<  std::endl;
    std::cout << getTime() << std::endl;
    auto a = arctern::map_match::snap_to_road(roads_binary, projections);
    std::cout << getTime() << std::endl;
    auto result_1 = std::static_pointer_cast<arrow::BinaryArray>(a[0]);
    std::cout << result_1->length() << std::endl;

    for (int32_t i = 0; i < result_1->length(); i++) {
        std::string geo = result_1->GetString(i);
        OGRGeometry *gps_point = nullptr;
        auto err_code = OGRGeometryFactory::createFromWkb(geo.c_str(),
                                                          nullptr,
                                                          &gps_point);
        if (err_code != OGRERR_NONE) throw nullptr;
        auto gps_point1 = dynamic_cast<OGRPoint *>(gps_point);
