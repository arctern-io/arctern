...
TEST(geometry_test, test_ST_Buffer){
  ...
  std::string input_polygon = "POLYGON((0 0,1 0,1 1,0 0))";
  OGRGeometry *rst_polygon;
  OGRGeometry *expect_polygon;

  OGRGeometryFactory::createFromWkt(buffer_polygon.c_str(), nullptr, (OGRGeometry**)(&rst_polygon));
  OGRGeometryFactory::createFromWkt(input_polygon.c_str(), nullptr, (OGRGeometry**)(&expect_polygon));
  OGRGeometry *exp = expect_polygon->Buffer(1.2);
  //ASSERT_TRUE(OGR_G_Equals(exp, rst_polygon));
  char *output;
  exp->exportToWkt(&output);
  ASSERT_EQ(output, buffer_polygon);
  OGRGeometry *expect_polygon2;
  OGRGeometryFactory::createFromWkt(output, nullptr, (OGRGeometry**)(&expect_polygon2));
  ASSERT_TRUE(OGR_G_Equals(expect_polygon2, rst_polygon));
