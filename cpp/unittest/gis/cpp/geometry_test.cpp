...
TEST(geometry_test, test_ST_Buffer){
  ...
  std::string input_polygon_str = "POLYGON ((0 0,1 1,1 0,0 0))";
  ...
  OGRGeometryFactory::createFromWkt(input_polygon_str.c_str(), nullptr, (OGRGeometry**)(&expect_polygon));

  OGRGeometry* exp = expect_polygon->Buffer(1.2);
  ASSERT_TRUE(rst_polygon->Equals(exp));
