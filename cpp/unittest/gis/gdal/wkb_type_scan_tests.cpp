...
TEST(wkb_type_scan, single_type_scan) {
  XYSpaceWkbCases cases;
  auto geo_cases = cases.GetAllCases();
  arctern::gis::dispatch::WkbTypeScanner scanner(geo_cases);
...
TEST(wkb_type_scan, unknown_type) {
  XYSpaceWkbCases cases;
  auto geo_cases = cases.GetAllCases();
  arctern::gis::dispatch::WkbTypeScanner scanner(geo_cases);
  scanner.mutable_types().push_back({WkbTypes::kLineString});
...
TEST(wkb_type_scan, unique_type) {
  XYSpaceWkbCases cases;
  auto geo_cases = cases.GetAllCases();
  {
    arctern::gis::dispatch::WkbTypeScanner scanner(geo_cases);
  ...
  }
  {
    ...
    arctern::gis::dispatch::WkbTypeScanner scanner(geo_cases2);
    scanner.mutable_types().push_back({WkbTypes::kLineString});
    auto type_masks = scanner.Scan();
...
TEST(wkb_type_scan, grouped_type) {
  XYSpaceWkbCases cases;
  auto geo_cases = cases.GetAllCases();
  arctern::gis::dispatch::WkbTypeScanner scanner(geo_cases);
...
TEST(wkb_type_scan, unique_grouped_type) {
  XYSpaceWkbCases cases;
...
TEST(wkb_type_scan, merge_and_split) {
  ...
  auto output = GenericArrayMerge<arrow::BinaryArray>({tmps[0], tmps[1]}, masks);
  checker(output, strs);
}

//TEST(wkb_type_scan, dispatch) {
//  using std::string;
//  using std::vector;
...
//  auto cases = arctern::gis::StrsToWkb(cases_raw);
//
...
//
//  auto true_checker = [&](std::shared_ptr<arrow::StringArray> wkb) {
//    EXPECT_EQ(wkb->length(), 1);
//    EXPECT_EQ(wkb->GetView(0), cases->GetView(2));
//    return wkb;
//  };
//
//  auto false_checker = [](std::shared_ptr<arrow::StringArray> wkb) {
