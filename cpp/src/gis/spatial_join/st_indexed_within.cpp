...
const std::vector<std::shared_ptr<arrow::Array>> join(
    ...
    for (int i = 0; i < array_size; i++) {
        ...
        for (int j = i * size_per_array; j < (i + 1) * size_per_array && j < num_of_point; j++) {
...
std::vector<std::shared_ptr<arrow::Array>> ST_IndexedWithin(
    ...
    if (!index_type.compare("RTREE")) {
        auto index = std::static_pointer_cast<RTree>(index_builder(polygons, IndexType::rTree));
        return join<RTree>(points,index);
    } else if (!index_type.compare("QuadTREE")) {
        auto index = std::static_pointer_cast<QuadTree>(index_builder(polygons, IndexType::qTree));
