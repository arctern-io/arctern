...
class VegaSquareMap : public Vega {
 public:
 public:
  VegaSquareMap() = default;

  explicit VegaSquareMap(const std::string& json);

  // TODO: add Build() api to build a vega json string.
  // std::string Build() final;

  int square_size() { return square_size_;}

  int square_spacing() { return square_spacing_;}
