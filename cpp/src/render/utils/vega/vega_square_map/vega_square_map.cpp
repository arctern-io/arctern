...
#include "render/utils/vega/vega_square_map/vega_square_map.h"

namespace arctern {
namespace render {

VegaSquareMap::VegaSquareMap(const std::string& json) { Parse(json); }

void VegaSquareMap::Parse(const std::string& json) {
  ...
  if (!JsonLabelCheck(mark_enter, "square_size") ||
      !JsonLabelCheck(mark_enter["square_size"], "value") ||
      !JsonTypeCheck(mark_enter["square_size"]["value"], rapidjson::Type::kNumberType)) {
    return;
  }
  square_size_ = mark_enter["square_size"]["value"].GetInt();

  if (!JsonLabelCheck(mark_enter, "square_spacing") ||
      !JsonLabelCheck(mark_enter["square_spacing"], "value") ||
      !JsonTypeCheck(mark_enter["square_spacing"]["value"], rapidjson::Type::kNumberType)) {
    return;
  }
  square_spacing_ = mark_enter["square_spacing"]["value"].GetInt();
