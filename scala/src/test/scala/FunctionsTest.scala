...
class FunctionsTest extends AdapterTest {
  def assertFuzzyAt(index: Int)(pair: (Row, String)): Unit = {
    val (row, ref_text) = pair
    val geom = row.getAs[Geometry](index)
    if (ref_text != "invalid") {
      assert(geom != null)
      val ref_geom = GeometryUDT.FromWkt(ref_text)
      val fuzzy_equal = geom.equalsExact(ref_geom, 10e-9)
      // enable detailed information
      assert(fuzzy_equal || geom.toString == ref_text)
    } else {
      assert(geom == null)
    }
  }
  ...
  test("ST_Buffer") {
    ...
    collect.zip(ref_data).foreach(assertFuzzyAt(0))

    val rst2 = df.select(st_buffer(col("geo"), lit(1)))
    rst2.show(false)
    val collect2 = rst2.collect()
    collect2.zip(ref_data).foreach(assertFuzzyAt(0))
  }
  ...
  test("ST_Length") {
    val data = Seq(
      ...
      Row(GeometryUDT.FromWkt("POLYGON EMPTY")),
      Row(GeometryUDT.FromWkt("MULTIPOLYGON ( ((0 0, 1 0, 1 1,0 0)) )")),
      Row(GeometryUDT.FromWkt("POLYGON ((1 1, 3 1, 3 3, 1 3, 1 1))")),
      Row(GeometryUDT.FromWkt("MULTIPOLYGON (((1 1,1 2,2 2,2 1,1 1)),((0 0,1 -1,1 1,-2 3,0 0)))")),
      Row(GeometryUDT.FromWkt("GEOMETRYCOLLECTION(POINT(4 6), POLYGON ((0 0, 1 0, 1 1, 0 1, 0 0)), LINESTRING(2 2, 2 3))")),
      // TODO: add curve support
      // Row(GeometryUDT.FromWkt("CURVEPOLYGON(CIRCULARSTRING(0 0, 4 0, 4 4, 0 4, 0 0),(1 1, 3 3, 3 1, 1 1))")),
    )
    ...
    assert(collect(5).getDouble(0) == 0.0)
    assert(collect(6).getDouble(0) == 0.0)
    assert(collect(7).getDouble(0) == 1.0)
    ...
    assert(collect2(5).getDouble(0) == 0.0)
    assert(collect2(6).getDouble(0) == 0.0)
    assert(collect2(7).getDouble(0) == 1.0)
  }
  ...
  test("ST_Transform") {
    ...
    val ref_data = Seq("POINT (-8235544.280259263 4974337.520006327)"
      , "POINT (-8251391.611649103 4947867.503249774)")
  ...
  test("ST_Transform-Null") {
    ...
    val ref_data = Seq("invalid"
      , "POINT (-8236547.714149274 4972580.886197065)"
      , "POLYGON ((0 -0.0000000007081155, 4452779.631730943 -0.0000000007081155, 4452779.631730943 4865942.279503176, 0 4865942.279503176, 0 -0.0000000007081155))"
      , "POINT (-8235544.280259263 4974337.520006327)"
    )
    collect.zip(ref_data).foreach(assertFuzzyAt(0))
